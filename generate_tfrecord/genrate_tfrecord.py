#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# @Use : python genrate_tfrecord.py 
# -i  xx/test_example 
# -o xx/test_example/tf_save 
"""
...
from generate_tfrecord import tf_io_pipline_tools
...
def init_args():
    ...
    parser.add_argument('-i', '--dataset_dir', default='', type=str, help='The source nsfw data dir path')
    parser.add_argument('-o', '--tfrecords_dir', default='', type=str, help='The dir path to save converted tfrecords')
...
class LaneNetDataProducer(object):
    ...
    def _generate_training_example_index_file(self):
        ...
        def _gather_example_info():
            ...
            for _gt_image_path in glob.glob('{:s}/*.jpg'.format(self._gt_image_dir)):
                _gt_binary_image_name = ops.split(_gt_image_path)[1]
                print(_gt_binary_image_name)
                _gt_binary_image_path = ops.join(self._gt_binary_image_dir, _gt_binary_image_name.split('.')[0]+'.png')
...
if __name__ == '__main__':
    ...
    producer.generate_tfrecords(save_dir=args.tfrecords_dir, step_size=50)
